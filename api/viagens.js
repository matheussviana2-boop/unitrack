...
function findArray(value, depth = 0) {
  ...
  const preferred = ['data','viagens','viagem','linhas','result','results','items','list','retorno'];
  ...
  try {
    ...
    respostas.forEach((r, index) => {
      ...
      try {
        ...
        if (!arr) {
          info.ok = false;
          info.erro = 'Formato JSON sem lista de viagens';
    ...
    if (meta.sucessos === 0) {
      res.status(502).json({ error: 'Nenhuma chave retornou viagens válidas', data: [], _meta: meta });
      return;
    }
